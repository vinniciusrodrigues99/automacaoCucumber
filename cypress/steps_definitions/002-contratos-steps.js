...
E("clica no módulo de {string}", (modulo) => {
  eventosContratos.acessarModulo(modulo);
});
E("clica na seção {string}", (nome) => {
  eventosContratos.acessarSeção(nome);
});
...
Entao("o sistema executa a ação e exibe a mensagem de {string}", (string) => {
  ...
  cy.screenshot('Evidência de sucesso do teste');
});
...
//caso34
Entao("a tabela retorna todos os campos do {string} em branco", (pagina)=>{
